...
@app.get("/health")
async def health():
    return {"ok": True, "app": APP_NAME, "version": "2.0", "mode": "research"}
...
async def scan(interval: str = DEFAULT_INTERVAL, limit: int = 30):
    ...
    for symbol in symbols:
        try:
            ...
            rows.append({"symbol": symbol, **s, "best_signal": best_name, "score": s[best_name], "label": label(s[best_name]), "data_provider": candle_provider})
        except Exception as e:
            rows.append({"symbol": symbol, "error": str(e), "data_provider": provider})
    rows.sort(key=lambda r: r.get("score", 0), reverse=True)
    return {"version": "2.0", "interval": interval, "ticker_provider": provider, "results": rows}
...
async def coin(symbol: str, interval: str = DEFAULT_INTERVAL, limit: int = 300):
    ...
    s = scores(f)
    entry = f["close"]
    return {"version": "2.0", "symbol": symbol.upper(), "interval": interval, "data_provider": provider, "features": f, "scores": s, "labels": {k: label(v) for k, v in s.items()}, "risk_example": {"entry": entry, "long_stop": dynamic_stop(entry, f["atr"], "long")}, "ai": explain(symbol.upper(), s, f)}
